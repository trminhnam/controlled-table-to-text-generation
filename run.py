...
def train(arg):
    ...
    train_loader, validation_loader, test_loader = create_dataloaders(
        arg, training_set, validation_set, test_set
    )
    ...
    for epoch in np.arange(arg.num_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            end1 = time.time()
            print(
                "(train) Epoch %d, batch %d, loss %0.4f, avg_score %0.4f, time %0.3f mins"
                % (
                    epoch,
                    batch_idx,
                    loss.item(),
        ...
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(validation_loader):
                ...
                end1 = time.time()
                print(
                    "(val) Epoch %d, batch %d, loss %0.4f, avg_score %0.4f, time %0.3f mins"
                    % (
                        epoch,
                        batch_idx,
                        loss.item(),
            ...
            print(
                "--> Epoch %d, train loss %0.4f (%s %0.4f), val loss %0.4f (%s %0.4f), time on train+val set %0.3f mins"
                % (
                    epoch,
                    train_loss,
                    arg.metric,
            ...
            if best_model_loss > val_loss:
                print(
                    f"Got a new best model with loss {val_loss}, previous best was {best_model_loss}."
                )
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            end1 = time.time()
            print(
                "(test) batch %d, loss %0.4f, avg_score %0.4f, time %0.3f mins"
        ...
        print(
            "--> Best Model %0.4f, Best Test loss %0.4f (%s %0.4f), time on test set %0.3f mins"
            % (
                best_model_loss,
                test_loss,
                arg.metric,
...
def generate_sentences(arg):
    enter_at = time.time()
    assert (
        arg.mode == "generate_sentences"
    ), f"arg.mode must be set to generate_sentences"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", type=str, choices=["train", "generate_sentences"], required=True
    )
    ...
    parser.add_argument(
        "--input_string",
        type=str,
        default="subtable_str_plus_subtable_metadata_str",
        choices=["subtable_str_plus_subtable_metadata_str", "raw_table"],
    )
    parser.add_argument("--metric", type=str, default="BLEU", choices=["BLEU", "ROUGE"])
    parser.add_argument(
        "--task", type=str, default="TOTTO", choices=["TOTTO", "cnn_dailymail"]
    )
